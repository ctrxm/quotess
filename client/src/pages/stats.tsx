...
export default function Stats() {
  ...
  if (authLoading || isLoading) return (
    <div className="max-w-lg mx-auto px-4 py-8 flex flex-col gap-4">
      {[1, 2, 3].map(i => <div key={i} className="border-4 border-black dark:border-[#555] rounded-xl h-32 bg-gray-100 dark:bg-[#2a2a28] animate-pulse shadow-[6px_6px_0px_black] dark:shadow-[6px_6px_0px_#333]" />)}
    </div>
  );
  if (!user) return null;
  ...
  const statCards = [
    { label: "Total Quote", value: stats.totalQuotes, icon: FileText, color: "bg-[#FFF3B0] dark:bg-[#3d3a20]" },
    { label: "Total Like", value: stats.totalLikes, icon: Heart, color: "bg-[#FFB3B3] dark:bg-[#4a2020]" },
    { label: "Total View", value: stats.totalViews, icon: Eye, color: "bg-[#B8DBFF] dark:bg-[#203040]" },
    { label: "Total Reaksi", value: stats.totalReactions, icon: Flame, color: "bg-[#FFD1A9] dark:bg-[#3d2a1a]" },
  ];

  return (
    <div className="max-w-lg mx-auto px-4 py-8" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
      <Link href="/profile">
        <button className="mb-6 flex items-center gap-2 font-bold text-sm border-2 border-black dark:border-[#555] px-4 py-2 rounded-lg bg-white dark:bg-[#2a2a28] text-black dark:text-[#f5f0e0] shadow-[3px_3px_0px_black] dark:shadow-[3px_3px_0px_#333] hover:shadow-[1px_1px_0px_black] hover:translate-x-[2px] hover:translate-y-[2px] transition-all" data-testid="button-back-stats">
          <ArrowLeft className="w-4 h-4" /> Kembali
        </button>
      </Link>

      <div className="border-4 border-black dark:border-[#555] rounded-xl bg-[#DDB8FF] dark:bg-[#2d1f3d] p-5 shadow-[8px_8px_0px_black] dark:shadow-[8px_8px_0px_#333] mb-6">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {statCards.map((card) => {
          ...
          return (
            <div key={card.label} className={`border-3 border-black dark:border-[#555] rounded-xl ${card.color} p-4 shadow-[5px_5px_0px_black] dark:shadow-[5px_5px_0px_#333]`} data-testid={`stat-${card.label.toLowerCase().replace(/\s/g, "-")}`}>
      ...
      {streak && (
        <div className="border-4 border-black dark:border-[#555] rounded-xl bg-[#C1F0C1] dark:bg-[#1a3d1a] p-5 shadow-[6px_6px_0px_black] dark:shadow-[6px_6px_0px_#333] mb-6">
      ...
      {stats.bestQuote && (
        <div className="border-4 border-black dark:border-[#555] rounded-xl bg-white dark:bg-[#2a2a28] p-5 shadow-[6px_6px_0px_black] dark:shadow-[6px_6px_0px_#333] mb-6">
          <h2 className="font-black text-sm uppercase tracking-widest mb-3 text-black dark:text-[#f5f0e0] flex items-center gap-2">
            <Trophy className="w-4 h-4" /> Quote Terpopuler
          </h2>
          <Link href={`/q/${stats.bestQuote.id}`}>
            <div className="border-2 border-black dark:border-[#555] rounded-lg p-4 bg-[#FFF3B0] dark:bg-[#3d3a20] cursor-pointer hover:shadow-[2px_2px_0px_black] transition-all" data-testid="card-best-quote">
      ...
      <div className="border-4 border-black dark:border-[#555] rounded-xl bg-white dark:bg-[#2a2a28] p-5 shadow-[6px_6px_0px_black] dark:shadow-[6px_6px_0px_#333]">
